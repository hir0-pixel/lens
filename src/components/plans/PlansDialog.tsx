  ...
  return (
    <Modal open={open} onClose={onClose} title="Plans & Credits" subtitle="Usage-based credits. 1 credit is about 1 English word of AI output." size="xl">
      <div className="p-6">
        <Card className="mb-6 border border-primary/30 bg-primary/10">
          <CardContent className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span>
                <span className="block type-caption font-semibold">Current plan: Pro</span>
                <span className="block type-caption text-muted-foreground">{credits.toLocaleString()} credits remaining. Resets in 12 days.</span>
              </span>
            </div>
          ...
          {PLANS.map((plan) => (
            <Card key={plan.name} className={cn("relative gap-0", plan.highlight && "border border-primary/60 bg-primary/5")}>
              <CardContent className="flex h-full flex-col">
                ...
                <div className="mt-2 flex items-baseline gap-1"><span className="type-display-md">${plan[billing]}</span><span className="type-caption text-muted-foreground">/mo</span></div>
                <div className="mt-1 flex items-center gap-1 type-caption text-muted-foreground"><Zap className="h-3 w-3 text-primary" />{plan.creditsPerMonth} credits</div>
                <Separator className="my-3" />
                <div className="space-y-1.5">
                  {plan.features.map((feature) => <div key={feature} className="flex items-center gap-1.5 type-caption text-muted-foreground"><Check className="h-3 w-3 text-[var(--success)]" />{feature}</div>)}
                </div>
                <Button type="button" disabled variant={plan.highlight ? "default" : "outline"} className="mt-4 w-full type-caption">
        ...
        <p className="mt-4 text-center type-caption text-muted-foreground">Credits are usage-based. Unused credits expire at the end of each billing cycle.</p>
      </div>
    </Modal>
  );
}
