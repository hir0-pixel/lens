...
const ragSource = readFileSync(path.join(root, "services/orchestrator/RagComposition.ts"), "utf8");
const schema = JSON.parse(readFileSync(path.join(root, "services/retrieval/retrieval-schema.json"), "utf8"));
for (const requirement of ["refreshCitation", "authorizeContextUse", "useBoundary: \"generation\" | \"citation\"", "status: \"no_context\"", "STALE_AUTHORITY", "maxContextBytes"]) {
  if (!ragSource.includes(requirement)) throw new Error(`M06 RAG composition is missing required control: ${requirement}`);
}
...
console.log("M06 retrieval, cache-control, and RAG composition preflight passed.");
