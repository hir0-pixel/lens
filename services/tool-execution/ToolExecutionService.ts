...
export interface Sandbox { dispatch(input: { targetRef: string; action: string; credentialRef: string; executionFence: string; idempotencyKey: string; argumentsDigest: string; arguments?: Record<string, unknown> }): Promise<{ status: "succeeded" | "unknown"; result?: { content: string; resourceRefs: readonly string[] } }>; }
export class ToolExecutionService {
  ...
  async execute(input: { idempotencyKey: string; subjectRef: string; toolName: string; toolVersion: string; argumentsDigest: string; executionFence: string; approvalRef?: string; arguments?: Record<string, unknown> }): Promise<ToolState> {
    ...
    try { outcome = await this.sandbox.dispatch({ targetRef: tool.targetRef, action: tool.action, credentialRef: credential.credentialRef, executionFence: input.executionFence, idempotencyKey: input.idempotencyKey, argumentsDigest: input.argumentsDigest, ...(input.arguments !== undefined ? { arguments: input.arguments } : {}) }); } catch { throw new ToolExecutionError("DEPENDENCY_UNAVAILABLE"); }
