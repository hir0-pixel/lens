...
import { createIdentitySessionGateway, requireInternalIssuer, requireInternalModelBridge } from "../../scripts/identity-session-gateway-core.mjs";

const secret = "a".repeat(32);
const make = (fetcher) => createIdentitySessionGateway({ mode: "internal-test-only", issuer: "https://identity.platform.internal:8443/realms/lens-internal", clientId: "lens-session-gateway", clientSecret: secret, allowedClientIp: "10.164.13.233", allowedOrigin: "http://localhost:1420", redirectUri: "https://lens-gateway.platform.internal:8444/auth/callback", modelBridgeUrl: "http://host.docker.internal:8080/v1/lab/generate", modelBridgeToken: "b".repeat(32), random: (n) => Buffer.alloc(n, 7), fetcher });
...
test("identity gateway accepts only the approved container-to-host model bridge", () => {
  assert.equal(requireInternalModelBridge("http://host.docker.internal:8080/v1/lab/generate"), "http://host.docker.internal:8080/v1/lab/generate");
...
test("identity gateway refuses unapproved browser origins and callback locations", () => {
  assert.throws(() => createIdentitySessionGateway({ mode: "internal-test-only", issuer: "https://identity.platform.internal:8443/realms/lens-internal", clientId: "client", clientSecret: secret, allowedClientIp: "10.164.13.233", allowedOrigin: "https://public.example", redirectUri: "https://lens-gateway.platform.internal:8444/auth/callback", modelBridgeUrl: "http://host.docker.internal:8080/v1/lab/generate", modelBridgeToken: secret }));
  assert.throws(() => createIdentitySessionGateway({ mode: "internal-test-only", issuer: "https://identity.platform.internal:8443/realms/lens-internal", clientId: "client", clientSecret: secret, allowedClientIp: "10.164.13.233", allowedOrigin: "http://localhost:1420", redirectUri: "https://public.example/auth/callback", modelBridgeUrl: "http://host.docker.internal:8080/v1/lab/generate", modelBridgeToken: secret }));
});
