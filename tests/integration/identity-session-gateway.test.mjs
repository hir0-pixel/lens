...
const make = (fetcher) => createIdentitySessionGateway({ mode: "internal-test-only", issuer: "https://identity.platform.internal:8443/realms/lens-internal", clientId: "lens-session-gateway", clientSecret: secret, allowedClientIp: "10.164.13.233", allowedOrigin: "http://localhost:1420", redirectUri: "https://lens-gateway.platform.internal:8444/auth/callback", modelBridgeUrl: "http://edge:8082/v1/lab/generate", modelBridgeToken: "b".repeat(32), random: (n) => Buffer.alloc(n, 7), fetcher });
...
test("identity gateway refuses unapproved browser origins and callback locations", () => {
  assert.throws(() => createIdentitySessionGateway({ mode: "internal-test-only", issuer: "https://identity.platform.internal:8443/realms/lens-internal", clientId: "client", clientSecret: secret, allowedClientIp: "10.164.13.233", allowedOrigin: "https://public.example", redirectUri: "https://lens-gateway.platform.internal:8444/auth/callback", modelBridgeUrl: "http://edge:8082/v1/lab/generate", modelBridgeToken: secret }));
  assert.throws(() => createIdentitySessionGateway({ mode: "internal-test-only", issuer: "https://identity.platform.internal:8443/realms/lens-internal", clientId: "client", clientSecret: secret, allowedClientIp: "10.164.13.233", allowedOrigin: "http://localhost:1420", redirectUri: "https://public.example/auth/callback", modelBridgeUrl: "http://edge:8082/v1/lab/generate", modelBridgeToken: secret }));
});
