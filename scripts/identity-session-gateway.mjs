...
const sessionCookie = (value) => `lens_internal_session=${value}; HttpOnly; Secure; SameSite=None; Path=/; Max-Age=1800`;
const server = createServer(async (request, response) => {
  const path = new URL(request.url ?? "/", "http://gateway.internal").pathname;
  if (request.method === "GET" && path === "/health/live") return write(response, 200, { status: "ok" });
  ...
  if (request.method === "GET" && path === "/auth/callback") {
    try {
      ...
      response.writeHead(302, { location: allowedOrigin, "set-cookie": sessionCookie(session.sessionId), "cache-control": "no-store" });
      return response.end();
  ...
  if (request.method === "GET" && path === "/v1/session") {
    const session = await gateway.bootstrapSession(cookie(request, "lens_internal_session"));
    return session ? write(response, 200, { subjectRef: session.subjectRef, csrfToken: session.csrfToken, expiresAt: session.expiresAt }, { ...headers, "set-cookie": sessionCookie(session.cookieValue) }) : write(response, 401, { error: "UNAUTHENTICATED" }, headers);
  }
  if (request.method === "POST" && path === "/auth/logout") {
    const session = await gateway.session(cookie(request, "lens_internal_session"), request.headers["x-lens-csrf"]);
    if (!session) return write(response, 401, { error: "UNAUTHENTICATED" }, headers);
    await gateway.revoke(cookie(request, "lens_internal_session"));
  ...
  if (request.method === "POST" && path === "/v1/generate") {
    try {
      ...
      return write(response, result.status, result.body, result.cookieValue ? { ...headers, "set-cookie": sessionCookie(result.cookieValue) } : headers);
    } catch (error) {
