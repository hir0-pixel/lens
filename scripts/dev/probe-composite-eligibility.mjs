...
import { SingleDigestModelEligibility, EmployeeCatalogModelEligibility, CompositeModelEligibility } from "../../orchestrator-service/src/service.ts";
...
const composite = new CompositeModelEligibility([
  new SingleDigestModelEligibility(pinned, () => Date.now()),
  new EmployeeCatalogModelEligibility(catalog),
]);
console.log("pinned route", await composite.resolveEndpoint({ capability: "rag-route-classification", artifactDigest: pinned, denyEpoch: 0 }));
console.log("employee model", await composite.resolveEndpoint({ capability: "grounded-assistant", artifactDigest: resolved.artifactDigest, denyEpoch: 0 }));
